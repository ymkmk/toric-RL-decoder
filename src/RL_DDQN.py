...
class RL_DDQN():
    def __init__(self, Network, Network_name, system_size=int, p_error=0.1, replay_memory_capacity=int, learning_rate=0.00025,
        ...
        if Network == ResNet18 or Network == ResNet34 or Network == ResNet50 or Network == ResNet101 or Network == ResNet152:
            self.q_function = self.network()
        else:
            self.q_function = self.network(system_size, number_of_actions, device)
    ...
    def save_network(self, PATH):
        torch.save(self.q_function, PATH)


    def load_network(self, PATH):
        self.q_function = torch.load(PATH, map_location='cpu')
        self.target_net = deepcopy(self.q_function)
        self.q_function = self.q_function.to(self.device)
        self.target_net = self.target_net.to(self.device)
    ...
    def experience_replay(self, criterion, optimizer, batch_size):
        ...
        #policy netって書いてあるけどqの間違い
        output = self.q_function(batch_state)
    ...
    def get_target_network_output(self, batch_next_state, batch_size):
        ...
        next_actions_index = np.zeros(batch_size)
        self.target_net.eval()
        self.q_function.eval()
        # init matrices
        batch_network_output = np.zeros(batch_size)
        ...
        for i in range(batch_size):
            if (batch_next_state[i].cpu().sum().item() == 0):
                ...
                # select greedy action 
                with torch.no_grad():        
                    net_output = self.q_function(perspectives)
                    q_values_table = np.array(net_output.cpu())
                    row, col = np.where(q_values_table == np.max(q_values_table))
                    if action_index[i] == None:
                        next_actions_index[i] = col[0]   
                        batch_network_output[i] = q_values_table[row[0], col[0]] 
                    elif action_index[i] != None:
                        next_actions_index[i] = col[0]   
                        action_from_policy_net = int(action_index[i])
        ...
        with torch.no_grad():
            ...
            val = self.q_function(batch_next_state)

            #print('next_action_index:{}'.format(next_actions_index))
            #print('main_q:{}'.format(val))
            result = np.zeros(batch_size)
            for i in range(batch_size):
                result[i] = val[i,int(next_actions_index[i])]
            results = torch.Tensor(result)
            #print('main_q:{}'.format(results))
            #print('target_q:{}'.format(target_output))
    ...
    def train(self, training_steps=int, target_update=int, epsilon_start=1.0, num_of_epsilon_steps=10, 
        ...
        if optimizer == 'RMSprop':
            optimizer = optim.RMSprop(self.q_function.parameters(), lr=self.learning_rate)
        elif optimizer == 'Adam':    
            optimizer = optim.Adam(self.q_function.parameters(), lr=self.learning_rate)
        # init counters
        steps_counter = 0
        update_counter = 1
        iteration = 0
    ...
    def select_action(self, number_of_actions=int, epsilon=float, grid_shift=int):
        ...
        if(1 - epsilon > rand):
            # select greedy action 
            with torch.no_grad():        
                policy_net_output = self.q_function(batch_perspectives)
