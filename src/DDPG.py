...
class DDPG():
    ...
    def save_network(self, PATH):
        torch.save(self.actor, PATH+'_actor')
        torch.save(self.critic, PATH+'_critic')


    def load_network(self, PATH):
        self.actor = torch.load(PATH+'_actor', map_location='cpu')
        self.target_actor = deepcopy(self.actor)
        self.actor = self.actor.to(self.device)
        self.target_actor = self.target_actor.to(self.device)

        self.critic = torch.load(PATH*'_critic', map_location='cpu')
        self.target_critic = deepcopy(self.critic)
        self.critic = self.actor.to(self.device)
        self.target_critic = self.target_critic.to(self.device)


    def experience_replay(self, actor_criterion, critic_criterion, actor_optimizer, critic_optimizer, batch_size):
        self.actor.train()
    ...
    def get_network_output_next_state(self, batch_next_state=float, batch_size=int, action_index=None):
        ...
        for i in range(batch_size):
            if (batch_next_state[i].cpu().sum().item() == 0):
                ...
                with torch.no_grad():        
                    net_output = self.target_actor(perspectives)
    ...
    def get_target_actor_output(self, batch_next_state, batch_size):
        with torch.no_grad():
    ...
    def train(self, training_steps=int, target_update=int, epsilon_start=1.0, num_of_epsilon_steps=10, 
        ...
        if optimizer == 'RMSprop':
            actor_optimizer = optim.RMSprop(self.actor.parameters(), lr=self.learning_rate)
        elif optimizer == 'Adam':    
        ...
        while iteration < training_steps:
            ...
            while terminal_state == 1 and num_of_steps_per_episode < self.max_nbr_actions_per_episode and iteration < training_steps:
                ...
                if steps_counter > replay_start_size:
                    update_counter += 1
                    self.experience_replay(actor_criterion,
                                            critic_criterion,
                                            actor_optimizer,
                                            critic_optimizer,
                                            batch_size)
    ...
    def select_action(self, number_of_actions=int, epsilon=float, grid_shift=int):
        ...
        if(1 - epsilon > rand):
            # select greedy action 
            with torch.no_grad():        
                policy_net_output = self.actor(batch_perspectives)
    ...
    def update_actor(self, criterion, optimizer, y, output, weights, indices):
        loss_critic = self.get_loss(criterion, optimizer, y, output, weights, indices)
        # backpropagate loss
        loss_critic.backward()
        critic_optimizer.step()
        #

    def update_critic(self, criterion, optimizer, y, output, weights, indices):
        loss_critic = self.get_loss(critic_criterion, critic_optimizer, y, output, weights, indices)
        # backpropagate loss
        loss_critic.backward()
        critic_optimizer.step()
